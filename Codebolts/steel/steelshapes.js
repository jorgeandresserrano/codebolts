...
var X0,
	...
	AMERICAN_SECTION_TYPES, DEFAULT_SECTION_TYPES;
...
function initializeVariables(){
	...
	AMERICAN_SECTION_TYPES = ['W','M','S','HP','C','MC','L','WT','MT','ST','2L','HSS','PIPE'];
	DEFAULT_SECTION_TYPES = ['W'];
	currentSpecName = CANADIAN_SPEC_NAME;
	
	currentShapes = canadianShapes;
	renderSectionTypeRibbon();
...
function renderSectionTypeRibbon(){
	var types = currentSpecName == AMERICAN_SPEC_NAME ? AMERICAN_SECTION_TYPES : DEFAULT_SECTION_TYPES;
	var items = '';
	
	for (var i = 0; i < types.length; i++) {
		var selectedClass = types[i] == 'W' ? ' on' : '';
		items += "<span class='section-type" + selectedClass + "'>" + types[i] + "</span>";
	}
	
	$('#section-type-ribbon').html(items);
}
